...
REPO="homebrew-merge-dependabot"
FORMULA="Formula/merge-dependabot.rb"
SCRIPT="merge-dependabot"

# ── Auto-increment patch version from current script ─────────────────────────
CURRENT=$(grep '^VERSION = ' "$SCRIPT" | cut -d'"' -f2)
...
echo ""
echo "${BOLD}🚀 Releasing merge-dependabot ${TAG}${RESET} (was ${CURRENT})"
echo ""
...
if [[ -z "$SHA" ]]; then
  echo "${RED}Error:${RESET} Could not fetch tarball from ${TARBALL_URL}."
  exit 1
fi
...
echo ""
echo "${GREEN}${BOLD}✅ Done! merge-dependabot ${TAG} is live.${RESET}"
echo ""
echo "Upgrade with:"
echo "  ${BOLD}brew update && brew upgrade merge-dependabot${RESET}"
echo ""
